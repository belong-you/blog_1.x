  ...
  *createNum () {  // 生成器函数传参毫无意义
  ...
  interpretMd(str) {
    ...
    for (const prop in arr) {
      ...
      // 多级标题
      if (/^#/.test(item)) {
        /^#\s/.test(item) && (arr[prop] = `<h1>${item.slice(2, num)}</h1>`);
      ...
      /https:\/\//.test(item) && (arr[prop] = item);
      /http:\/\//.test(item) && (arr[prop] = item);
      ...
      if (/\s\[\s/.test(item)) {
        ...
        // console.log(link[0].slice(2, -1))
        // console.log(item)
        arr[prop] = item.replace(/\[\s/, `<a href='${link[0].slice(2, -1)}' target='block'>`).replace(/\s\]\(.{0,100}/, '</a>');
      }
    }
    // console.log(arr)
