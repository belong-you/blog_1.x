...
export default {
	data () {
		return {
			...
			hash: this.$route.hash.slice(1),  // url hash
			tag: this.$route.params.id,  // 标签
			activeName: '',
			html: '',  // 转义后的html结构
			options: [],  // 下拉菜单
			href: location.href,
	...
	async created () {
		...
		// 判断数据中有没有相应的数据
		if (this.hash === '') this.activeName = this.note[this.tag][0].hash;
		else this.activeName = this.hash;

		this.getText();  // 获取对应的 .md 文件
		...
		// 		this.activeName = '1';
		// 	} else {
		// 		this.activeName = this.$route.hash.slice(1)
		// 	}
		// 	this.getText();  // 重新发送请求
		// });
	...
	methods: {
		...
		async getText () {
			...
			const data = await axios.getNoteMd(this.tag, this.activeName).then(res => {
				return res;
			}, () => {
				this.html = '';  // 请求不到让内容为空
				this.$notify({
